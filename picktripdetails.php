...
$deli_address = "Yes"; //will be delivered to address on file
...
if(isset($_POST['deliver'])) { // if where to deliver button is pressed
		...
		$select_address = mysqli_query($connection, $custQuery);
				$custQuery = "INSERT INTO cust_alt_address (cust_id, cust_alt_addr ) ";
				$custQuery .= "VALUES ('{$cust_id}', '{$deli_address}' )";
		*/	
...
if($wheredel == "Nominate") {
	...
	$custQuery = "SELECT * FROM cust_alt_address";
	$custQuery = "INSERT INTO cust_alt_address (cust_id, cust_add1, cust_add2, cust_city, cust_municipality, cust_deli_fee) ";
	$custQuery .= "VALUES ('{$cust_id}', '{$add1}', '{$add2}', '{$citydeliver}', '{$munideliver}', '{$zone_fee}' )";
...
if(isset($_POST['submitagree'])) {  //if I Agree button was pressed
	...
	$cust_deli_pick = "Delivery";
	$cust_rental_status = "For Review";

	$custQuery = "SELECT * FROM custrentinfo";
	$custQuery = "INSERT INTO custrentinfo (cust_id, cust_car_id, cust_reserve_date, car_drive, cust_pickup_date, cust_return_date, cust_deli_pick, cust_delitoaddress, cust_howmany_days, cust_rental_status, cust_rent_amt, cust_deli_fee ) ";
	$custQuery .= "VALUES ('{$cust_id}', '{$the_car_id}', now(), '{$cdrive}', '{$indate}', '{$retdate}', '{$cust_deli_pick}', '{$deli_address}', '{$diff_date}', '{$cust_rental_status}', '{$cardayratepeak}', '{$zone_fee}' )";
...
header("location: picktripdetailsconfirm.php?zonefee={$zone_fee}&munideliver={$munideliver}&citydeliver={$citydeliver}&add1={$add1}&add2={$add2}&wheredeliver={$wheredel}&edit={$the_car_id}&carid={$the_car_id}&carmodel={$carmodel}&cardesc={$cardesc}&cartrans={$cartrans}&carcap={$carcapacity}&wdrive={$cdrive}&begdate={$indate}&retdate={$retdate}&totalrate={$cardayratepeak}&wdrive={$cdrive}&logsw={$loginsw}&custid={$cust_id}"); 
...
<?php
	...
	//$passwordsw = $_GET['ispassword']; // 02/11!!! to check if username and password is existing to see if customer is new for delivery purposes
            ...
            <section class="tm-banner">
                <div class="tm-container-outer tm-banner-bg">
                    <div style="background: url(img/about-bg.jpg) center top no-repeat; background-attachment: fixed;" class="container">
						...
						<?php
							echo "<form action='picktripdetails.php?zonefee={$zone_fee}&wheredeliver={$wheredel}&citydeliver={$citydeliver}&muni_deliver={$munideliver}&zonefee={$zone_fee}&edit={$the_car_id}&carid={$carid}&carmodel={$carmodel}&cardesc={$cardesc}&cartrans={$cartrans}&carcap={$carcapacity}&wdrive={$cdrive}&begdate={$indate}&retdate={$retdate}&totalrate={$cardayratepeak}&logsw={$loginsw}&custid={$cust_id}' method='post' class='tm-search-form tm-section-pad-2'>";
						...
						?>
							...
							//Ask where customer wants to have the vehicle delivered 
								...
								<!---check if this is a better place to save info that car will be delivered to address on file---->
									</div>
...
<!------------->			<?php if($t_and_csw == 0) { ?>				
							...
							<?php } else {?> <!--IF MUNICIPALITY AND CITY IS NOT YET selected-->
								...
								<div class="form-row tm-search-form-row"> 
									...
									<div class="form-group tm-form-group tm-form-group-pad tm-form-group-1">
                                        <label for="inputCity">City (select one)</label>
								...
								if($munideliversw == 0 AND $citydeliversw == 1 ) {
									...
									<div class="form-group tm-form-group tm-form-group-pad tm-form-group-1">
                                        <label for="inputCity">Municipality (select one)</label>
							...
							<?php } ?> <!--  /.IF MUNICIPALITY AND CITY IS selected-->
								...
								<?php //if($munideliversw == 1 AND $citydeliversw == 1) { ?>
								<!--
								<button type="submit" name="checkavail" class="btn btn-warning tm-btn tm-btn-search text-uppercase" id="btnSubmit">Submit</button>
								<h6>Delivery fee: <strong>Php 700.00</strong></h6>
								<hr>
								-->
								<?php// } ?>
	...
	<!--------->			<?php } ?> <!--  /.t_and_csw	  -->			
							...
							<?php if($t_and_csw == 1) { ?>
								...
								<!--<div class="form-row tm-search-form-row">
								<hr>
								<h6><strong>Your vehicle will be delivered to <?php echo $wheredelmsg; ?>. Please take note of the Terms and Conditions below.</strong><br></h6>
								</div>-->
